...
SITE_URL = "https://berkeleyjuggling.org/"
# This is the URL where Nikola's output will be deployed.
# If not set, defaults to SITE_URL
# BASE_URL = "https://berkeleyjuggling.org/"
BLOG_EMAIL = "info@berkeleyjuggling.org"
BLOG_DESCRIPTION = "The home page of the Berkeley Juggling Club"  # (translatable)
...
# the environment.
# Junk that must never make the round trip.  .DS_Store is created by the
# Finder; rss.xml is generated by Nikola into output/ for every gallery, so a
...
DEPLOY_COMMANDS = {
    "default": [
        ...
        # in a fresh checkout before 'nikola deploy download'.  The risk is
        # not to R2 -- 'copy' below never deletes -- but to the site: building
        # without media publishes pages whose images all 404.
        ...
        f"rclone copy images/    bjc-r2:bjc-media/images/    {RCLONE_EXCLUDES} --progress",
        f"rclone copy galleries/ bjc-r2:bjc-media/galleries/ {RCLONE_EXCLUDES} --progress",
        f"rclone copy videos/    bjc-r2:bjc-media/videos/    {RCLONE_EXCLUDES} --progress",
        # Publish the built site.  Video is not in output/ at all -- it is
        # served straight from R2 by the video shortcode, because
        # roundabout.mp4 was 79 MiB against Pages' 25 MiB per-file cap (it is
        # 39 MiB since being re-encoded from HEVC to H.264, still over).
    ...
    "download": [
        ...
        f"rclone copy bjc-r2:bjc-media/images/    images/    {RCLONE_EXCLUDES} --progress",
        f"rclone copy bjc-r2:bjc-media/galleries/ galleries/ {RCLONE_EXCLUDES} --progress",
        f"rclone copy bjc-r2:bjc-media/videos/    videos/    {RCLONE_EXCLUDES} --progress",
    ],
}
